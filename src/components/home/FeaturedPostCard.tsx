...
export const FeaturedPostCard: FC<IFeaturedPostCardProps> = ({ post, isAdmin, onSelectMemory, onEdit, onDelete }) => {
  ...
  return (
    <motion.article
      ...
      className="card-float rounded-xl overflow-hidden bg-surface cursor-pointer"
    >
      <img
        src={post.image}
        ...
        fetchPriority="high"
        decoding="async"
        className="w-full aspect-[4/3] sm:aspect-video lg:aspect-[21/9] lg:max-h-[420px] object-cover"
      />
      <div className="p-6 sm:p-8">
        <h2 className="text-xl sm:text-3xl font-bold text-white">{post.displayTitle}</h2>
        <p className="mt-2 sm:mt-3 text-gray-300 whitespace-pre-wrap line-clamp-2 sm:line-clamp-none">
